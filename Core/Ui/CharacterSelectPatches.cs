...
internal static class CharacterSelectGateImpl
{
    ...
    internal static void Refresh(NCharacterSelectScreen screen)
    {
        ...
        button.Text = CoopLobbyData.MyVote(lobby) ? TextLeave : TextJoin;
    }
    ...
    private static void OnPressed(NCharacterSelectScreen screen)
    {
        ...
        if (CoopLobbyData.MyVote(lobby))
        {
            // 退出合作：先撤票，再走本体的"取消准备"（它会把选人界面、官方确认键都恢复回来）。
            CoopLobbyData.SetMyVote(lobby, wants: false);
        ...
        {
            // 加入合作：投自己那一票（per-player 槽位，SyncLobbyOnChange 会自动推给主机），
            // 然后走本体的"确认"（等价于按下官方确认键：收起选人界面、切到"已准备"、等其他人）。
            // 名单不需要在这里等同步：它进 run snapshot，开局时两端读的是同一份（见 CoopLobbyData）。
            CoopLobbyData.SetMyVote(lobby, wants: true);
    ...
    internal static void OnBodyUnready(NCharacterSelectScreen screen)
    {
        ...
        var localId = lobby.LocalPlayer.id;
        if (localId == 0UL || !CoopLobbyData.MyVote(lobby))
        {
            return;
        }
