...
/// <summary>把牌列表 / 牌堆排成<b>两端一致</b>的顺序，让"随机取牌"两端取到同一张。</summary>
/// <remarks>
/// 本体的两种洗牌都<b>依赖输入顺序</b>：
/// <c>UnstableShuffle</c>（Fisher-Yates）：输入顺序不同 → 结果不同；
/// <c>StableShuffle</c>：虽然先 <c>list.Sort()</c> 抹平顺序，但 <c>List.Sort</c> 是<b>不稳定排序</b>，
/// 而 <c>CardModel</c> 之间大量存在"比较相等"的牌（同名牌、同升级），它们的先后仍取决于输入顺序 → 结果仍可能不同。
...
/// <b>三条口径</b>：<see cref="Sort" /> = 洗牌前排序（本体随后会自己再洗一次）；
/// <see cref="SortPile" /> = <b>随机取牌前</b>排序，直接改牌堆本身 —— 两端都改 → 不只是这次取到同一张，
/// 连"之后从堆顶抽牌"的顺序也一起收敛。破灭（<c>HAVOC</c>）/ 横祸（<c>CATASTROPHE</c>）/ 乱战（<c>UPROAR</c>）
/// 这类"打出随机牌"的效果都走这里（接点见 <c>Core/Patches/Together/Deck/RandomPickPatches.cs</c>）；
/// <see cref="SortedCopy" /> = 给"候选池"排序（<c>TakeRandom</c> 内部是 <c>UnstableShuffle</c>，最怕输入顺序不同）。
/// </remarks>
internal static class DeterministicCardOrder
{
    ...
    /// <summary><b>随机取牌之前</b>把这个牌堆本身排成两端一致（就地改）。</summary>
    /// <remarks>
