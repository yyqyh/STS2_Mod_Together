...
/// <remarks>
/// <b>只对"共享卡组里的牌"这么做</b>（见 <see cref="SharedDeckRegistry" />）：并发失效是"同一张牌被两个人同时选"造成的，
/// 那是我们这边的锅；别的 mod 自己的牌失效时把手拿开，让它按本体抛异常（不吞它的控制流）。
/// </remarks>
...
internal static class EnchantApplyGuardPatch
{
    ...
    private static bool Prefix(EnchantmentModel enchantment, CardModel card, ref EnchantmentModel? __result)
    {
        ...
        if (!SharedDeckRegistry.IsShared(card))
        {
            CappedLog.Info(
                "event.conflict",
                $"附魔失效但不属于共享卡组（{DeterministicCardOrder.DescribeCards([card], 1)}）→ 交回本体处理");
            return true;
        }
...
internal static class RemoveFromDeckGuardPatch
{
    ...
    private static bool Prefix(ref IReadOnlyList<CardModel> cards, ref Task __result)
    {
        ...
        // 失效项里没有一张是共享卡组的牌 → 不是我们造成的并发冲突，交回本体（保留别处"用异常中止"的语义）。
        if (!cards.Any(card => SharedDeckRegistry.IsShared(card)))
        {
            CappedLog.Info("event.conflict", $"从卡组移除：失效的牌都不属于共享卡组 → 交回本体处理（{cards.Count} 张）");
            return true;
        }

        CappedLog.Info("event.conflict", $"从卡组移除：跳过已失效的牌 {cards.Count} → {valid.Count}（共享卡组并发）");
...
internal static class TransformGuardPatch
{
        ...
        ref Task<IEnumerable<CardPileAddResult>> __result)
    {
        ...
        // 失效项里没有一张是共享卡组的牌 → 不是我们造成的并发冲突，交回本体。
        if (!all.Any(t => SharedDeckRegistry.IsShared(t.Original)))
        {
            CappedLog.Info("event.conflict", $"变牌：失效的牌都不属于共享卡组 → 交回本体处理（{all.Count} 张）");
            return true;
        }

        CappedLog.Info(
            "event.conflict",
            $"变牌跳过已失效的牌：{all.Count} → {valid.Count}（共享卡组并发：另一份事件实例先动过它）");
