...
internal static class PopulateCombatStateAnchorOnlyPatch
{
    ...
    private static bool Prefix(Player __instance)
    {
        ...
        // 兼容开关关掉时照常填充（等于回到"两个人各填一遍"），只在别的 mod 自己接管回声牌堆时用。
        return !TogetherPair.IsEcho(__instance)
               || !Together.Core.Settings.TogetherSettingsSync.EffectiveCompatEchoPopulateSkip;
    }
...
/// <summary>进阶之灾去重：共享卡组下本体"逐玩家各加一张"的诅咒会变成两张 —— 只摘掉回声那次调用新加的那张。</summary>
/// <remarks>
/// <c>AscensionManager.ApplyEffectsTo(player)</c> 逐玩家调用（<c>RunManager.InitializeNewRun</c> 里
/// <c>foreach (player) ApplyAscensionEffects(player)</c>），里面那句
/// <c>player.Deck.AddInternal(AscendersBane, -1, silent: true)</c> 自然执行两次；而共享卡组下两人的
/// <c>player.Deck</c> 指向同一份（<see cref="TogetherPair.Arm" /> 已换掉回声的 <c>Deck</c> 字段 + getter 重定向）
/// → 同一张诅咒被加两遍，开局卡组里就有两张"进阶之灾"。
/// <para>
/// <b>为什么不做成"Prefix 直接跳过回声那次调用"</b>：这个方法里还有<b>应当逐玩家生效</b>的部分
/// （<c>SubtractFromMaxPotionCount</c>，药水格各算各的），整段跳过会让回声的药水格比锚点多一个。
/// 想只拦"加诅咒"那一段就得复刻本体逻辑（本体以后加了第三项我们就会漏），所以不划算。
/// </para>
/// <para>
/// <b>为什么用"调用前后作差"而不是"扫一遍只留第一张"</b>：作差只摘<b>这次调用新加进来的那些对象</b>，
/// 别的路径（别的 mod / 存档带来的）加的诅咒一根汗毛都不碰 —— 兼容性更好。
/// 读档 / 重连那条老路（存档里可能已经躺着两张）由 <see cref="TogetherPair" /> 里的
/// <see cref="DedupeSharedDeck" /> 兜底。
/// </para>
/// 摘牌要连 <c>RunState</c> 的登记一起清（<c>RunState.RemoveCard</c>），否则全卡表里留一张"无主的牌"。
/// 判卡靠类型 + ID 双保险：两样都对不上就什么都不做 —— 绝不去删不认识的牌。
/// </remarks>
...
internal static class AscensionBaneDedupePatch
{
    ...
    /// <summary>调用前记下"共享卡组里本来就有哪些进阶之灾"（按对象引用），只对回声那次调用记账。</summary>
    [HarmonyPrefix]
    private static void Prefix(Player __0, ref HashSet<CardModel>? __state)
    {
        if (!TogetherPair.IsActive || TogetherPair.Anchor is not { } anchor || !TogetherPair.IsEcho(__0))
        {
            return;
        }

        __state = BanersIn(anchor.Deck);
    }

    /// <summary>调用后只摘掉"这次调用新加进来的"那些 —— 幂等，别的来源的诅咒不动。</summary>
    [HarmonyPostfix]
    private static void Postfix(Player __0, HashSet<CardModel>? __state)
    {
        if (__state is null || !TogetherPair.IsActive || TogetherPair.Anchor is not { } anchor)
        {
            return;
        }

        var extras = anchor.Deck.Cards
            .Where(card => IsAscendersBane(card) && !__state.Contains(card))
            .ToList();
        if (extras.Count == 0)
        {
            return;
        ...
        {
            anchor.Deck.RemoveInternal(extra, silent: true);
            anchor.RunState.RemoveCard(extra);
        }

        Log.Info(
            $"[together] 进阶之灾收口：摘掉回声那次调用新加的 {extras.Count} 张诅咒"
            + $"（共享卡组 {anchor.Deck.Cards.Count} 张）");
    }

    /// <summary>这口卡组里现有的进阶之灾（按对象引用）。</summary>
    private static HashSet<CardModel> BanersIn(CardPile deck)
    {
        var found = new HashSet<CardModel>(ReferenceEqualityComparer.Instance);
...
internal static class DeterministicCardComparePatch
{
    ...
    private static void Postfix(CardModel __instance, AbstractModel? other, ref int __result)
    {
        if (__result != 0
            ...
            || !Together.Core.Settings.TogetherSettingsSync.EffectiveCompatDeterministicOrder   // ★ 兼容模式开关
            || other is not CardModel otherCard)
        {
            return;
        }
...
/// <summary>初始洗牌（<c>CardPile.RandomizeOrderInternal</c>）：<b>只打顺序指纹，不动牌堆</b>。</summary>
/// <remarks>
...
/// <b>我们不再替它把牌堆排齐</b>（那会覆盖"控制牌堆顺序"的 mod），改成洗完留一条两端可比的指纹：
/// 真出现"战斗一开始就分叉"，先看这行 —— 指纹不同就是输入顺序先漂了。
/// 另：这里只挂具体的非泛型方法。**不要**去挂 <c>ListExtensions.UnstableShuffle&lt;T&gt;</c> 这类泛型洗牌方法：
...
internal static class InitialShuffleProbePatch
{
    /// <summary><c>CardPile.Cards</c> 是只读包装，要读当前的顺序得直接拿这个列表。</summary>
    ...
    [HarmonyPostfix]
    private static void Postfix(CardPile __instance, Player player)
    {
        ...
        CappedLog.Info(
            "order.probe",
            $"初始洗牌后 {__instance.Type} 顺序指纹：{DeterministicCardOrder.Fingerprint(cards)}（两端应一致）");
    }
}

/// <summary>跑局就绪（新开一局 / 读档重连）：等 <c>RunState</c> 完全构造完之后再激活共享配对。</summary>
/// <remarks>
/// 绝不能提前激活——<c>CreateShared</c> 会在设置 <c>player.RunState</c> 之后
/// 遍历该玩家的卡组给每张卡设 owner，提前激活会让第二次遍历读到被重定向的卡组，
/// 从而"同一张牌设两次 owner"抛异常（实测开局黑屏就是这么来的）。
/// 两者的差别只在 <c>isNewRun</c>：只有新局才加血量上限 / 合并初始卡组。见 <see cref="TogetherPair.Arm" />。
/// </remarks>
[HarmonyPatch]
internal static class RunStateReadyPatch
{
    private static IEnumerable<MethodBase> TargetMethods()
    {
        yield return AccessTools.Method(typeof(RunState), nameof(RunState.CreateForNewRun));
        yield return AccessTools.Method(typeof(RunState), nameof(RunState.FromSerializable));
    }

    [HarmonyPostfix]
    private static void Postfix(RunState __result, MethodBase __originalMethod)
    {
        // 只有新开一局才加血量上限 / 合并初始卡组；读档重连加会让存档里的值越滚越大。
        TogetherPair.Arm(__result, isNewRun: __originalMethod.Name == nameof(RunState.CreateForNewRun));
    }
}
